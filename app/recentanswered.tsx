...
// --- Dummy Data ---
const DUMMY_AnsweredS: AnsweredItem[] = [
  {
    id: "1",
...
export default function App() {
  const [visibleAnsweredsCount, setVisibleAnsweredsCount] =
    useState(AnsweredS_PER_PAGE);

  const handleLoadMore = () => {
    setVisibleAnsweredsCount((prevCount) => prevCount + AnsweredS_PER_PAGE);
  };

  const displayedAnswereds = DUMMY_AnsweredS.slice(0, visibleAnsweredsCount);
  const canLoadMore = visibleAnsweredsCount < DUMMY_AnsweredS.length;
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <Text style={styles.recentAnsweredTitle}>Recent Answered</Text>

        {displayedAnswereds.map((Answered) => (
          <AnsweredCard key={Answered.id} item={Answered} />
        ))}
